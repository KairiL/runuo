...
namespace Server.Mobiles
{
	...
	public class BlacksmithRumor
	{
		...
		public BlacksmithRumor( string message, string keyword )
		{
			m_Message = message;
			m_Keyword = keyword;
		}

		public static BlacksmithRumor Deserialize( GenericReader reader )
		{
			if ( !reader.ReadBool() )
				return null;

			return new BlacksmithRumor( reader.ReadString(), reader.ReadString() );
		}

		public static void Serialize( GenericWriter writer, BlacksmithRumor rumor )
		{
			if ( rumor == null )
			{
				writer.Write( false );
	...
	public class ManageBlacksmithEntry : ContextMenuEntry
	{
		private Mobile m_From;
		private PlayerBlacksmith m_Blacksmith;
		...
		{
			m_Blacksmith.BeginManagement( m_From );
		}
	}

	public class PlayerBlacksmith : Blacksmith
	{
		private Mobile m_Owner;
		private BaseHouse m_House;
		private string m_TipMessage;

		private BlacksmithRumor[] m_Rumors;
		...
		public BlacksmithRumor[] Rumors{ get{ return m_Rumors; } }
		...
		public PlayerBlacksmith( Mobile owner, BaseHouse house ) : base()
		{
			m_Owner = owner;
			House = house;
			m_Rumors = new BlacksmithRumor[3];
		...
		public override void OnSpeech( SpeechEventArgs e )
		{
			...
			if ( !e.Handled && InRange( e.Mobile, 3 ) )
			{
				...
				{
					BlacksmithRumor rumor = m_Rumors[i];
		...
		public override void GetContextMenuEntries( Mobile from, List<ContextMenuEntry> list )
		{
			...
			if ( IsOwner( from ) && from.InLOS( this ) )
				list.Add( new ManageBlacksmithEntry( from, this ) );
		}
		...
		public void BeginManagement( Mobile from )
		{
			...
			from.SendGump( new BlacksmithGump( from, this ) );
		}
		...
		{
			from.SendGump( new BlacksmithTitleGump( from, this ) );
		}
		...
		{
			if ( Title == "the blacksmith" || Title == "the blacksmith" )
			{
		...
		public PlayerBlacksmith( Serial serial ) : base( serial )
		{
		}
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			for ( int i = 0; i < m_Rumors.Length; ++i )
				BlacksmithRumor.Serialize( writer, m_Rumors[i] );
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				...
				case 0:
				{
					...
					m_Rumors = new BlacksmithRumor[reader.ReadEncodedInt()];

					for ( int i = 0; i < m_Rumors.Length; ++i )
						m_Rumors[i] = BlacksmithRumor.Deserialize( reader );
	...
	public class BlacksmithTitleGump : Gump
	{
		private Mobile m_From;
		private PlayerBlacksmith m_Blacksmith;
			...
			if ( buttonID > 0 )
			{
				...
				if ( buttonID > 0 )
				{
					...
					if ( buttonID >= 0 && buttonID < m_Entries.Length )
						m_Blacksmith.EndChangeTitle( m_From, m_Entries[buttonID].m_Title, m_Entries[buttonID].m_Vendor );
				}
				else
				{
					m_Blacksmith.CancelChangeTitle( m_From );
				}
			}
		}
	}

	public class BlacksmithGump : Gump
	{
		private Mobile m_From;
		private PlayerBlacksmith m_Blacksmith;
		...
		public void RenderDismissConfirmation()
		{
			...
			AddHtml( 170, 160, 380, 20, "Are you sure you want to dismiss your blacksmith?", false, false );
		...
		private void RenderAppearanceCategories()
		{
			...
			if ( m_Blacksmith.BodyValue != 0x340 && m_Blacksmith.BodyValue != 0x402 ) {			
		...
		{
			if ( !m_Blacksmith.IsOwner( m_From ) )
				return;
			...
			switch ( type )
			{
				...
				case 5: // Appearance category selection
				{
					switch ( index )
					{
						case 0: m_Blacksmith.BeginChangeTitle( m_From ); break;
						case 1: m_Blacksmith.BeginChangeAppearance( m_From ); break;
						case 2: m_Blacksmith.ChangeGender( m_From ); break;
					}
