...
namespace Server.Mobiles
{
	...
	public class TannerRumor
	{
		...
		public TannerRumor( string message, string keyword )
		{
			m_Message = message;
			m_Keyword = keyword;
		}

		public static TannerRumor Deserialize( GenericReader reader )
		{
			if ( !reader.ReadBool() )
				return null;

			return new TannerRumor( reader.ReadString(), reader.ReadString() );
		}

		public static void Serialize( GenericWriter writer, TannerRumor rumor )
		{
			if ( rumor == null )
			{
				writer.Write( false );
	...
	public class ManageTannerEntry : ContextMenuEntry
	{
		private Mobile m_From;
		private PlayerTanner m_Tanner;

		public ManageTannerEntry( Mobile from, PlayerTanner tanner ) : base( 6151, 12 )
		{
			m_From = from;
			m_Tanner = tanner;
		}
		...
		{
			m_Tanner.BeginManagement( m_From );
		}
	}

	public class PlayerTanner : Tanner
	{
		private Mobile m_Owner;
		private BaseHouse m_House;
		private string m_TipMessage;

		private TannerRumor[] m_Rumors;
		...
		public TannerRumor[] Rumors{ get{ return m_Rumors; } }
		...
		public PlayerTanner( Mobile owner, BaseHouse house ) : base()
		{
			m_Owner = owner;
			House = house;
			m_Rumors = new TannerRumor[3];
		...
		public override void GetContextMenuEntries( Mobile from, List<ContextMenuEntry> list )
		{
			...
			if ( IsOwner( from ) && from.InLOS( this ) )
				list.Add( new ManageTannerEntry( from, this ) );
		}
		...
		public void BeginManagement( Mobile from )
		{
			...
			from.SendGump( new TannerGump( from, this ) );
		}
		...
		{
			from.SendGump( new TannerTitleGump( from, this ) );
		}
		...
		{
			if ( Title == "the tanner" || Title == "the tanner" )
			{
		...
		public PlayerTanner( Serial serial ) : base( serial )
		{
		}
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			for ( int i = 0; i < m_Rumors.Length; ++i )
				TannerRumor.Serialize( writer, m_Rumors[i] );
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				...
				case 0:
				{
					...
					m_Rumors = new TannerRumor[reader.ReadEncodedInt()];

					for ( int i = 0; i < m_Rumors.Length; ++i )
						m_Rumors[i] = TannerRumor.Deserialize( reader );
	...
	public class TannerTitleGump : Gump
	{
		private Mobile m_From;
		private PlayerTanner m_Tanner;
		...
		public TannerTitleGump( Mobile from, PlayerTanner tanner ) : base( 0, 0 )
		{
			m_From = from;
			m_Tanner = tanner;

			from.CloseGump( typeof( TannerGump ) );
			from.CloseGump( typeof( TannerTitleGump ) );
			...
			if ( buttonID > 0 )
			{
				...
				if ( buttonID > 0 )
				{
					...
					if ( buttonID >= 0 && buttonID < m_Entries.Length )
						m_Tanner.EndChangeTitle( m_From, m_Entries[buttonID].m_Title, m_Entries[buttonID].m_Vendor );
				}
				else
				{
					m_Tanner.CancelChangeTitle( m_From );
				}
			}
		}
	}

	public class TannerGump : Gump
	{
		private Mobile m_From;
		private PlayerTanner m_Tanner;
		...
		public void RenderDismissConfirmation()
		{
			...
			AddHtml( 170, 160, 380, 20, "Are you sure you want to dismiss your tanner?", false, false );
		...
		private void RenderAppearanceCategories()
		{
			...
			if ( m_Tanner.BodyValue != 0x340 && m_Tanner.BodyValue != 0x402 ) {			
		...
		public TannerGump( Mobile from, PlayerTanner tanner ) : base( 0, 0 )
		{
			m_From = from;
			m_Tanner = tanner;
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			switch ( type )
			{
				...
				case 5: // Appearance category selection
				{
					switch ( index )
					{
						case 0: m_Tanner.BeginChangeTitle( m_From ); break;
						case 1: m_Tanner.BeginChangeAppearance( m_From ); break;
						case 2: m_Tanner.ChangeGender( m_From ); break;
					}
