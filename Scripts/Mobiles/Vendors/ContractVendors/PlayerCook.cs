...
namespace Server.Mobiles
{
	...
	public class CookRumor
	{
		...
		public CookRumor( string message, string keyword )
		{
			m_Message = message;
			m_Keyword = keyword;
		}

		public static CookRumor Deserialize( GenericReader reader )
		{
			if ( !reader.ReadBool() )
				return null;

			return new CookRumor( reader.ReadString(), reader.ReadString() );
		}

		public static void Serialize( GenericWriter writer, CookRumor rumor )
		{
			if ( rumor == null )
			{
				writer.Write( false );
	...
	public class ManageCookEntry : ContextMenuEntry
	{
		private Mobile m_From;
		private PlayerCook m_Cook;
		...
		{
			m_Cook.BeginManagement( m_From );
		}
	}

	public class PlayerCook : Cook
	{
		private Mobile m_Owner;
		private BaseHouse m_House;
		private string m_TipMessage;

		private CookRumor[] m_Rumors;
		...
		public CookRumor[] Rumors{ get{ return m_Rumors; } }
		...
		public PlayerCook( Mobile owner, BaseHouse house ) : base()
		{
			m_Owner = owner;
			House = house;
			m_Rumors = new CookRumor[3];
		...
		public override void GetContextMenuEntries( Mobile from, List<ContextMenuEntry> list )
		{
			...
			if ( IsOwner( from ) && from.InLOS( this ) )
				list.Add( new ManageCookEntry( from, this ) );
		}
		...
		public void BeginManagement( Mobile from )
		{
			...
			from.SendGump( new CookGump( from, this ) );
		}
		...
		{
			from.SendGump( new CookTitleGump( from, this ) );
		}
		...
		{
			if ( Title == "the cook" || Title == "the cook" )
			{
		...
		public PlayerCook( Serial serial ) : base( serial )
		{
		}
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			for ( int i = 0; i < m_Rumors.Length; ++i )
				CookRumor.Serialize( writer, m_Rumors[i] );
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				...
				case 0:
				{
					...
					m_Rumors = new CookRumor[reader.ReadEncodedInt()];

					for ( int i = 0; i < m_Rumors.Length; ++i )
						m_Rumors[i] = CookRumor.Deserialize( reader );
	...
	public class CookTitleGump : Gump
	{
		private Mobile m_From;
		private PlayerCook m_Cook;
			...
			if ( buttonID > 0 )
			{
				...
				if ( buttonID > 0 )
				{
					...
					if ( buttonID >= 0 && buttonID < m_Entries.Length )
						m_Cook.EndChangeTitle( m_From, m_Entries[buttonID].m_Title, m_Entries[buttonID].m_Vendor );
				}
				else
				{
					m_Cook.CancelChangeTitle( m_From );
				}
			}
		}
	}

	public class CookGump : Gump
	{
		private Mobile m_From;
		private PlayerCook m_Cook;
		...
		public void RenderDismissConfirmation()
		{
			...
			AddHtml( 170, 160, 380, 20, "Are you sure you want to dismiss your cook?", false, false );
		...
		private void RenderAppearanceCategories()
		{
			...
			if ( m_Cook.BodyValue != 0x340 && m_Cook.BodyValue != 0x402 ) {			
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			switch ( type )
			{
				...
				case 5: // Appearance category selection
				{
					switch ( index )
					{
						case 0: m_Cook.BeginChangeTitle( m_From ); break;
						case 1: m_Cook.BeginChangeAppearance( m_From ); break;
						case 2: m_Cook.ChangeGender( m_From ); break;
					}
