...
namespace Server.Mobiles
{
	...
	public class AnimalTrainerRumor
	{
		...
		public AnimalTrainerRumor( string message, string keyword )
		{
			m_Message = message;
			m_Keyword = keyword;
		}

		public static AnimalTrainerRumor Deserialize( GenericReader reader )
		{
			if ( !reader.ReadBool() )
				return null;

			return new AnimalTrainerRumor( reader.ReadString(), reader.ReadString() );
		}

		public static void Serialize( GenericWriter writer, AnimalTrainerRumor rumor )
		{
			if ( rumor == null )
			{
				writer.Write( false );
	...
	public class ManageAnimalTrainerEntry : ContextMenuEntry
	{
		private Mobile m_From;
		private PlayerAnimalTrainer m_AnimalTrainer;

		public ManageAnimalTrainerEntry( Mobile from, PlayerAnimalTrainer trainer ) : base( 6151, 12 )
		{
			m_From = from;
			m_AnimalTrainer = trainer;
		}

		public override void OnClick()
		{
			m_AnimalTrainer.BeginManagement( m_From );
		}
	}

	public class PlayerAnimalTrainer : AnimalTrainer
	{
		private Mobile m_Owner;
		private BaseHouse m_House;
		private string m_TipMessage;

		private AnimalTrainerRumor[] m_Rumors;
		...
		public AnimalTrainerRumor[] Rumors{ get{ return m_Rumors; } }
		...
		public PlayerAnimalTrainer( Mobile owner, BaseHouse house ) : base()
		{
			m_Owner = owner;
			House = house;
			m_Rumors = new AnimalTrainerRumor[3];
		...
		public override void OnSpeech( SpeechEventArgs e )
		{
			...
			if ( !e.Handled && InRange( e.Mobile, 3 ) )
			{
				...
				{
					AnimalTrainerRumor rumor = m_Rumors[i];
		...
		public override void GetContextMenuEntries( Mobile from, List<ContextMenuEntry> list )
		{
			...
			if ( IsOwner( from ) && from.InLOS( this ) )
				list.Add( new ManageAnimalTrainerEntry( from, this ) );
		}
		...
		public void BeginManagement( Mobile from )
		{
			...
			from.SendGump( new AnimalTrainerGump( from, this ) );
		}
		...
		{
			from.SendGump( new AnimalTrainerTitleGump( from, this ) );
		}
		...
		{
			if ( Title == "the animal trainer" || Title == "the trainer" )
			{
		...
		public PlayerAnimalTrainer( Serial serial ) : base( serial )
		{
		}
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			for ( int i = 0; i < m_Rumors.Length; ++i )
				AnimalTrainerRumor.Serialize( writer, m_Rumors[i] );
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				...
				case 0:
				{
					...
					m_Rumors = new AnimalTrainerRumor[reader.ReadEncodedInt()];

					for ( int i = 0; i < m_Rumors.Length; ++i )
						m_Rumors[i] = AnimalTrainerRumor.Deserialize( reader );
	...
	public class AnimalTrainerTitleGump : Gump
	{
		private Mobile m_From;
		private PlayerAnimalTrainer m_AnimalTrainer;
		...
		public AnimalTrainerTitleGump( Mobile from, PlayerAnimalTrainer trainer ) : base( 0, 0 )
		{
			m_From = from;
			m_AnimalTrainer = trainer;

			from.CloseGump( typeof( AnimalTrainerGump ) );
			from.CloseGump( typeof( AnimalTrainerTitleGump ) );
			...
			if ( buttonID > 0 )
			{
				...
				if ( buttonID > 0 )
				{
					...
					if ( buttonID >= 0 && buttonID < m_Entries.Length )
						m_AnimalTrainer.EndChangeTitle( m_From, m_Entries[buttonID].m_Title, m_Entries[buttonID].m_Vendor );
				}
				else
				{
					m_AnimalTrainer.CancelChangeTitle( m_From );
				}
			}
		}
	}

	public class AnimalTrainerGump : Gump
	{
		private Mobile m_From;
		private PlayerAnimalTrainer m_AnimalTrainer;
		...
		public void RenderDismissConfirmation()
		{
			...
			AddHtml( 170, 160, 380, 20, "Are you sure you want to dismiss your trainer?", false, false );
		...
		private void RenderAppearanceCategories()
		{
			...
			if ( m_AnimalTrainer.BodyValue != 0x340 && m_AnimalTrainer.BodyValue != 0x402 ) {			
		...
		public AnimalTrainerGump( Mobile from, PlayerAnimalTrainer trainer ) : base( 0, 0 )
		{
			m_From = from;
			m_AnimalTrainer = trainer;
		...
		{
			if ( !m_AnimalTrainer.IsOwner( m_From ) )
				return;
			...
			switch ( type )
			{
				...
				case 5: // Appearance category selection
				{
					switch ( index )
					{
						case 0: m_AnimalTrainer.BeginChangeTitle( m_From ); break;
						case 1: m_AnimalTrainer.BeginChangeAppearance( m_From ); break;
						case 2: m_AnimalTrainer.ChangeGender( m_From ); break;
					}
