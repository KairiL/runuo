...
namespace Server.Mobiles
{
	...
	public class HealerRumor
	{
		...
		public HealerRumor( string message, string keyword )
		{
			m_Message = message;
			m_Keyword = keyword;
		}

		public static HealerRumor Deserialize( GenericReader reader )
		{
			if ( !reader.ReadBool() )
				return null;

			return new HealerRumor( reader.ReadString(), reader.ReadString() );
		}

		public static void Serialize( GenericWriter writer, HealerRumor rumor )
		{
			if ( rumor == null )
			{
				writer.Write( false );
	...
	public class ManageHealerEntry : ContextMenuEntry
	{
		private Mobile m_From;
		private PlayerHealer m_Healer;

		public ManageHealerEntry( Mobile from, PlayerHealer healer ) : base( 6151, 12 )
		{
			m_From = from;
			m_Healer = healer;
		}
		...
		{
			m_Healer.BeginManagement( m_From );
		}
	}

	public class PlayerHealer : Healer
	{
		private Mobile m_Owner;
		private BaseHouse m_House;
		private string m_TipMessage;

		private HealerRumor[] m_Rumors;
		...
		public HealerRumor[] Rumors{ get{ return m_Rumors; } }
		...
		public PlayerHealer( Mobile owner, BaseHouse house ) : base()
		{
			m_Owner = owner;
			House = house;
			m_Rumors = new HealerRumor[3];
		...
		public override void GetContextMenuEntries( Mobile from, List<ContextMenuEntry> list )
		{
			...
			if ( IsOwner( from ) && from.InLOS( this ) )
				list.Add( new ManageHealerEntry( from, this ) );
		}
		...
		public void BeginManagement( Mobile from )
		{
			...
			from.SendGump( new HealerGump( from, this ) );
		}
		...
		{
			from.SendGump( new HealerTitleGump( from, this ) );
		}
		...
		{
			if ( Title == "the healer" || Title == "the healer" )
			{
		...
		public PlayerHealer( Serial serial ) : base( serial )
		{
		}
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			for ( int i = 0; i < m_Rumors.Length; ++i )
				HealerRumor.Serialize( writer, m_Rumors[i] );
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				...
				case 0:
				{
					...
					m_Rumors = new HealerRumor[reader.ReadEncodedInt()];

					for ( int i = 0; i < m_Rumors.Length; ++i )
						m_Rumors[i] = HealerRumor.Deserialize( reader );
	...
	public class HealerTitleGump : Gump
	{
		private Mobile m_From;
		private PlayerHealer m_Healer;
		...
		public HealerTitleGump( Mobile from, PlayerHealer healer ) : base( 0, 0 )
		{
			m_From = from;
			m_Healer = healer;

			from.CloseGump( typeof( HealerGump ) );
			from.CloseGump( typeof( HealerTitleGump ) );
			...
			if ( buttonID > 0 )
			{
				...
				if ( buttonID > 0 )
				{
					...
					if ( buttonID >= 0 && buttonID < m_Entries.Length )
						m_Healer.EndChangeTitle( m_From, m_Entries[buttonID].m_Title, m_Entries[buttonID].m_Vendor );
				}
				else
				{
					m_Healer.CancelChangeTitle( m_From );
				}
			}
		}
	}

	public class HealerGump : Gump
	{
		private Mobile m_From;
		private PlayerHealer m_Healer;
		...
		public void RenderDismissConfirmation()
		{
			...
			AddHtml( 170, 160, 380, 20, "Are you sure you want to dismiss your healer?", false, false );
		...
		private void RenderAppearanceCategories()
		{
			...
			if ( m_Healer.BodyValue != 0x340 && m_Healer.BodyValue != 0x402 ) {			
		...
		public HealerGump( Mobile from, PlayerHealer healer ) : base( 0, 0 )
		{
			m_From = from;
			m_Healer = healer;
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			switch ( type )
			{
				...
				case 5: // Appearance category selection
				{
					switch ( index )
					{
						case 0: m_Healer.BeginChangeTitle( m_From ); break;
						case 1: m_Healer.BeginChangeAppearance( m_From ); break;
						case 2: m_Healer.ChangeGender( m_From ); break;
					}
