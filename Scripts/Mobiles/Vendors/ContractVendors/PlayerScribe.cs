...
namespace Server.Mobiles
{
	...
	public class ScribeRumor
	{
		...
		public ScribeRumor( string message, string keyword )
		{
			m_Message = message;
			m_Keyword = keyword;
		}

		public static ScribeRumor Deserialize( GenericReader reader )
		{
			if ( !reader.ReadBool() )
				return null;

			return new ScribeRumor( reader.ReadString(), reader.ReadString() );
		}

		public static void Serialize( GenericWriter writer, ScribeRumor rumor )
		{
			if ( rumor == null )
			{
				writer.Write( false );
	...
	public class ManageScribeEntry : ContextMenuEntry
	{
		private Mobile m_From;
		private PlayerScribe m_Scribe;
		...
		{
			m_Scribe.BeginManagement( m_From );
		}
	}

	public class PlayerScribe : Scribe
	{
		private Mobile m_Owner;
		private BaseHouse m_House;
		private string m_TipMessage;

		private ScribeRumor[] m_Rumors;
		...
		public ScribeRumor[] Rumors{ get{ return m_Rumors; } }
		...
		public PlayerScribe( Mobile owner, BaseHouse house ) : base()
		{
			m_Owner = owner;
			House = house;
			m_Rumors = new ScribeRumor[3];
		...
		public override void GetContextMenuEntries( Mobile from, List<ContextMenuEntry> list )
		{
			...
			if ( IsOwner( from ) && from.InLOS( this ) )
				list.Add( new ManageScribeEntry( from, this ) );
		}
		...
		public void BeginManagement( Mobile from )
		{
			...
			from.SendGump( new ScribeGump( from, this ) );
		}
		...
		{
			from.SendGump( new ScribeTitleGump( from, this ) );
		}
		...
		{
			if ( Title == "the scribe" || Title == "the scribe" )
			{
		...
		public PlayerScribe( Serial serial ) : base( serial )
		{
		}
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			for ( int i = 0; i < m_Rumors.Length; ++i )
				ScribeRumor.Serialize( writer, m_Rumors[i] );
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				...
				case 0:
				{
					...
					m_Rumors = new ScribeRumor[reader.ReadEncodedInt()];

					for ( int i = 0; i < m_Rumors.Length; ++i )
						m_Rumors[i] = ScribeRumor.Deserialize( reader );
	...
	public class ScribeTitleGump : Gump
	{
		private Mobile m_From;
		private PlayerScribe m_Scribe;
			...
			if ( buttonID > 0 )
			{
				...
				if ( buttonID > 0 )
				{
					...
					if ( buttonID >= 0 && buttonID < m_Entries.Length )
						m_Scribe.EndChangeTitle( m_From, m_Entries[buttonID].m_Title, m_Entries[buttonID].m_Vendor );
				}
				else
				{
					m_Scribe.CancelChangeTitle( m_From );
				}
			}
		}
	}

	public class ScribeGump : Gump
	{
		private Mobile m_From;
		private PlayerScribe m_Scribe;
		...
		public void RenderDismissConfirmation()
		{
			...
			AddHtml( 170, 160, 380, 20, "Are you sure you want to dismiss your scribe?", false, false );
		...
		private void RenderAppearanceCategories()
		{
			...
			if ( m_Scribe.BodyValue != 0x340 && m_Scribe.BodyValue != 0x402 ) {			
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			switch ( type )
			{
				...
				case 5: // Appearance category selection
				{
					switch ( index )
					{
						case 0: m_Scribe.BeginChangeTitle( m_From ); break;
						case 1: m_Scribe.BeginChangeAppearance( m_From ); break;
						case 2: m_Scribe.ChangeGender( m_From ); break;
					}
