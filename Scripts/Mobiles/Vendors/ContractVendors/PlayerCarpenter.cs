...
namespace Server.Mobiles
{
	...
	public class CarpenterRumor
	{
		...
		public CarpenterRumor( string message, string keyword )
		{
			m_Message = message;
			m_Keyword = keyword;
		}

		public static CarpenterRumor Deserialize( GenericReader reader )
		{
			if ( !reader.ReadBool() )
				return null;

			return new CarpenterRumor( reader.ReadString(), reader.ReadString() );
		}

		public static void Serialize( GenericWriter writer, CarpenterRumor rumor )
		{
			if ( rumor == null )
			{
				writer.Write( false );
	...
	public class ManageCarpenterEntry : ContextMenuEntry
	{
		private Mobile m_From;
		private PlayerCarpenter m_Carpenter;

		public ManageCarpenterEntry( Mobile from, PlayerCarpenter carpenter ) : base( 6151, 12 )
		{
			m_From = from;
		...
		{
			m_Carpenter.BeginManagement( m_From );
		}
	}

	public class PlayerCarpenter : Carpenter
	{
		private Mobile m_Owner;
		private BaseHouse m_House;
		private string m_TipMessage;

		private CarpenterRumor[] m_Rumors;
		...
		public CarpenterRumor[] Rumors{ get{ return m_Rumors; } }
		...
		public PlayerCarpenter( Mobile owner, BaseHouse house ) : base()
		{
			m_Owner = owner;
			House = house;
			m_Rumors = new CarpenterRumor[3];
		...
		public override void OnSpeech( SpeechEventArgs e )
		{
			...
			if ( !e.Handled && InRange( e.Mobile, 3 ) )
			{
				...
				{
					CarpenterRumor rumor = m_Rumors[i];
		...
		public override void GetContextMenuEntries( Mobile from, List<ContextMenuEntry> list )
		{
			...
			if ( IsOwner( from ) && from.InLOS( this ) )
				list.Add( new ManageCarpenterEntry( from, this ) );
		}
		...
		public void BeginManagement( Mobile from )
		{
			...
			from.SendGump( new CarpenterGump( from, this ) );
		}
		...
		{
			from.SendGump( new CarpenterTitleGump( from, this ) );
		}
		...
		{
			if ( Title == "the carpenter" || Title == "the carpenter" )
			{
		...
		public PlayerCarpenter( Serial serial ) : base( serial )
		{
		}
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			for ( int i = 0; i < m_Rumors.Length; ++i )
				CarpenterRumor.Serialize( writer, m_Rumors[i] );
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				...
				case 0:
				{
					...
					m_Rumors = new CarpenterRumor[reader.ReadEncodedInt()];

					for ( int i = 0; i < m_Rumors.Length; ++i )
						m_Rumors[i] = CarpenterRumor.Deserialize( reader );
	...
	public class CarpenterTitleGump : Gump
	{
		private Mobile m_From;
		private PlayerCarpenter m_Carpenter;
		...
		public CarpenterTitleGump( Mobile from, PlayerCarpenter carpenter ) : base( 0, 0 )
		{
			m_From = from;
			m_Carpenter = carpenter;

			from.CloseGump( typeof( CarpenterGump ) );
			from.CloseGump( typeof( CarpenterTitleGump ) );
			...
			if ( buttonID > 0 )
			{
				...
				if ( buttonID > 0 )
				{
					...
					if ( buttonID >= 0 && buttonID < m_Entries.Length )
						m_Carpenter.EndChangeTitle( m_From, m_Entries[buttonID].m_Title, m_Entries[buttonID].m_Vendor );
				}
				else
				{
					m_Carpenter.CancelChangeTitle( m_From );
				}
			}
		}
	}

	public class CarpenterGump : Gump
	{
		private Mobile m_From;
		private PlayerCarpenter m_Carpenter;
		...
		public void RenderDismissConfirmation()
		{
			...
			AddHtml( 170, 160, 380, 20, "Are you sure you want to dismiss your carpenter?", false, false );
		...
		private void RenderAppearanceCategories()
		{
			...
			if ( m_Carpenter.BodyValue != 0x340 && m_Carpenter.BodyValue != 0x402 ) {			
		...
		public CarpenterGump( Mobile from, PlayerCarpenter carpenter ) : base( 0, 0 )
		{
			m_From = from;
			m_Carpenter = carpenter;
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			switch ( type )
			{
				...
				case 5: // Appearance category selection
				{
					switch ( index )
					{
						case 0: m_Carpenter.BeginChangeTitle( m_From ); break;
						case 1: m_Carpenter.BeginChangeAppearance( m_From ); break;
						case 2: m_Carpenter.ChangeGender( m_From ); break;
					}
