...
namespace Server.Mobiles
{
	...
	public class BowyerRumor
	{
		...
		public BowyerRumor( string message, string keyword )
		{
			m_Message = message;
			m_Keyword = keyword;
		}

		public static BowyerRumor Deserialize( GenericReader reader )
		{
			if ( !reader.ReadBool() )
				return null;

			return new BowyerRumor( reader.ReadString(), reader.ReadString() );
		}

		public static void Serialize( GenericWriter writer, BowyerRumor rumor )
		{
			if ( rumor == null )
			{
				writer.Write( false );
	...
	public class ManageBowyerEntry : ContextMenuEntry
	{
		private Mobile m_From;
		private PlayerBowyer m_Bowyer;

		public ManageBowyerEntry( Mobile from, PlayerBowyer bowyer ) : base( 6151, 12 )
		{
			m_From = from;
			m_Bowyer = bowyer;
		}
		...
		{
			m_Bowyer.BeginManagement( m_From );
		}
	}

	public class PlayerBowyer : Bowyer
	{
		private Mobile m_Owner;
		private BaseHouse m_House;
		private string m_TipMessage;

		private BowyerRumor[] m_Rumors;
		...
		public BowyerRumor[] Rumors{ get{ return m_Rumors; } }
		...
		public PlayerBowyer( Mobile owner, BaseHouse house ) : base()
		{
			m_Owner = owner;
			House = house;
			m_Rumors = new BowyerRumor[3];
		...
		public override void GetContextMenuEntries( Mobile from, List<ContextMenuEntry> list )
		{
			...
			if ( IsOwner( from ) && from.InLOS( this ) )
				list.Add( new ManageBowyerEntry( from, this ) );
		}
		...
		public void BeginManagement( Mobile from )
		{
			...
			from.SendGump( new BowyerGump( from, this ) );
		}
		...
		{
			from.SendGump( new BowyerTitleGump( from, this ) );
		}
		...
		{
			if ( Title == "the bowyer" || Title == "the bowyer" )
			{
		...
		public PlayerBowyer( Serial serial ) : base( serial )
		{
		}
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			for ( int i = 0; i < m_Rumors.Length; ++i )
				BowyerRumor.Serialize( writer, m_Rumors[i] );
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				...
				case 0:
				{
					...
					m_Rumors = new BowyerRumor[reader.ReadEncodedInt()];

					for ( int i = 0; i < m_Rumors.Length; ++i )
						m_Rumors[i] = BowyerRumor.Deserialize( reader );
	...
	public class BowyerTitleGump : Gump
	{
		private Mobile m_From;
		private PlayerBowyer m_Bowyer;
		...
		public BowyerTitleGump( Mobile from, PlayerBowyer bowyer ) : base( 0, 0 )
		{
			m_From = from;
			m_Bowyer = bowyer;

			from.CloseGump( typeof( BowyerGump ) );
			from.CloseGump( typeof( BowyerTitleGump ) );
			...
			if ( buttonID > 0 )
			{
				...
				if ( buttonID > 0 )
				{
					...
					if ( buttonID >= 0 && buttonID < m_Entries.Length )
						m_Bowyer.EndChangeTitle( m_From, m_Entries[buttonID].m_Title, m_Entries[buttonID].m_Vendor );
				}
				else
				{
					m_Bowyer.CancelChangeTitle( m_From );
				}
			}
		}
	}

	public class BowyerGump : Gump
	{
		private Mobile m_From;
		private PlayerBowyer m_Bowyer;
		...
		public void RenderDismissConfirmation()
		{
			...
			AddHtml( 170, 160, 380, 20, "Are you sure you want to dismiss your bowyer?", false, false );
		...
		private void RenderAppearanceCategories()
		{
			...
			if ( m_Bowyer.BodyValue != 0x340 && m_Bowyer.BodyValue != 0x402 ) {			
		...
		public BowyerGump( Mobile from, PlayerBowyer bowyer ) : base( 0, 0 )
		{
			m_From = from;
			m_Bowyer = bowyer;
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			switch ( type )
			{
				...
				case 5: // Appearance category selection
				{
					switch ( index )
					{
						case 0: m_Bowyer.BeginChangeTitle( m_From ); break;
						case 1: m_Bowyer.BeginChangeAppearance( m_From ); break;
						case 2: m_Bowyer.ChangeGender( m_From ); break;
					}
