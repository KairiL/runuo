...
namespace Server.Mobiles
{
	...
	public class BakerRumor
	{
		...
		public BakerRumor( string message, string keyword )
		{
			m_Message = message;
			m_Keyword = keyword;
		}

		public static BakerRumor Deserialize( GenericReader reader )
		{
			if ( !reader.ReadBool() )
				return null;

			return new BakerRumor( reader.ReadString(), reader.ReadString() );
		}

		public static void Serialize( GenericWriter writer, BakerRumor rumor )
		{
			if ( rumor == null )
			{
				writer.Write( false );
	...
	public class ManageBakerEntry : ContextMenuEntry
	{
		private Mobile m_From;
		private PlayerBaker m_Baker;

		public ManageBakerEntry( Mobile from, PlayerBaker baker ) : base( 6151, 12 )
		{
			m_From = from;
			m_Baker = baker;
		}
		...
		{
			m_Baker.BeginManagement( m_From );
		}
	}

	public class PlayerBaker : Baker
	{
		private Mobile m_Owner;
		private BaseHouse m_House;
		private string m_TipMessage;

		private BakerRumor[] m_Rumors;
		...
		public BakerRumor[] Rumors{ get{ return m_Rumors; } }
		...
		public PlayerBaker( Mobile owner, BaseHouse house ) : base()
		{
			m_Owner = owner;
			House = house;
			m_Rumors = new BakerRumor[3];
		...
		public override void GetContextMenuEntries( Mobile from, List<ContextMenuEntry> list )
		{
			...
			if ( IsOwner( from ) && from.InLOS( this ) )
				list.Add( new ManageBakerEntry( from, this ) );
		}
		...
		public void BeginManagement( Mobile from )
		{
			...
			from.SendGump( new BakerGump( from, this ) );
		}
		...
		{
			from.SendGump( new BakerTitleGump( from, this ) );
		}
		...
		{
			if ( Title == "the baker" || Title == "the baker" )
			{
		...
		public PlayerBaker( Serial serial ) : base( serial )
		{
		}
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			for ( int i = 0; i < m_Rumors.Length; ++i )
				BakerRumor.Serialize( writer, m_Rumors[i] );
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				...
				case 0:
				{
					...
					m_Rumors = new BakerRumor[reader.ReadEncodedInt()];

					for ( int i = 0; i < m_Rumors.Length; ++i )
						m_Rumors[i] = BakerRumor.Deserialize( reader );
	...
	public class BakerTitleGump : Gump
	{
		private Mobile m_From;
		private PlayerBaker m_Baker;
		...
		public BakerTitleGump( Mobile from, PlayerBaker baker ) : base( 0, 0 )
		{
			m_From = from;
			m_Baker = baker;

			from.CloseGump( typeof( BakerGump ) );
			from.CloseGump( typeof( BakerTitleGump ) );
			...
			if ( buttonID > 0 )
			{
				...
				if ( buttonID > 0 )
				{
					...
					if ( buttonID >= 0 && buttonID < m_Entries.Length )
						m_Baker.EndChangeTitle( m_From, m_Entries[buttonID].m_Title, m_Entries[buttonID].m_Vendor );
				}
				else
				{
					m_Baker.CancelChangeTitle( m_From );
				}
			}
		}
	}

	public class BakerGump : Gump
	{
		private Mobile m_From;
		private PlayerBaker m_Baker;
		...
		public void RenderDismissConfirmation()
		{
			...
			AddHtml( 170, 160, 380, 20, "Are you sure you want to dismiss your baker?", false, false );
		...
		private void RenderAppearanceCategories()
		{
			...
			if ( m_Baker.BodyValue != 0x340 && m_Baker.BodyValue != 0x402 ) {			
		...
		public BakerGump( Mobile from, PlayerBaker baker ) : base( 0, 0 )
		{
			m_From = from;
			m_Baker = baker;
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			switch ( type )
			{
				...
				case 5: // Appearance category selection
				{
					switch ( index )
					{
						case 0: m_Baker.BeginChangeTitle( m_From ); break;
						case 1: m_Baker.BeginChangeAppearance( m_From ); break;
						case 2: m_Baker.ChangeGender( m_From ); break;
					}
