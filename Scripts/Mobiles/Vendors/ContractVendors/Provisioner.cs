...
namespace Server.Mobiles
{
	...
	public class ProvisionerRumor
	{
		...
		public ProvisionerRumor( string message, string keyword )
		{
			m_Message = message;
			m_Keyword = keyword;
		}

		public static ProvisionerRumor Deserialize( GenericReader reader )
		{
			if ( !reader.ReadBool() )
				return null;

			return new ProvisionerRumor( reader.ReadString(), reader.ReadString() );
		}
	...
	public class ManageProvisionerEntry : ContextMenuEntry
	{
		private Mobile m_From;
		private PlayerProvisioner m_Provisioner;

		public ManageProvisionerEntry( Mobile from, PlayerProvisioner provisioner ) : base( 6151, 12 )
		{
			m_From = from;
		...
		{
			m_Provisioner.BeginManagement( m_From );
		}
	}

	public class PlayerProvisioner : Provisioner
	{
		private Mobile m_Owner;
		private BaseHouse m_House;
		private string m_TipMessage;

		private ProvisionerRumor[] m_Rumors;
		...
		public ProvisionerRumor[] Rumors{ get{ return m_Rumors; } }
		...
		public PlayerProvisioner( Mobile owner, BaseHouse house ) : base()
		{
			m_Owner = owner;
			House = house;
			m_Rumors = new ProvisionerRumor[3];
		...
		public override void GetContextMenuEntries( Mobile from, List<ContextMenuEntry> list )
		{
			...
			if ( IsOwner( from ) && from.InLOS( this ) )
				list.Add( new ManageProvisionerEntry( from, this ) );
		}
		...
		public void BeginManagement( Mobile from )
		{
			...
			from.SendGump( new ProvisionerGump( from, this ) );
		}
		...
		{
			from.SendGump( new ProvisionerTitleGump( from, this ) );
		}
		...
		{
			if ( Title == "the provisioner" || Title == "the provisioner" )
			{
		...
		public PlayerProvisioner( Serial serial ) : base( serial )
		{
		}
	...
	public class ProvisionerTitleGump : Gump
	{
		private Mobile m_From;
		private PlayerProvisioner m_Provisioner;
		...
		public ProvisionerTitleGump( Mobile from, PlayerProvisioner provisioner ) : base( 0, 0 )
		{
			m_From = from;
			m_Provisioner = provisioner;

			from.CloseGump( typeof( ProvisionerGump ) );
			from.CloseGump( typeof( ProvisionerTitleGump ) );
			...
			if ( buttonID > 0 )
			{
				...
				if ( buttonID > 0 )
				{
					...
					if ( buttonID >= 0 && buttonID < m_Entries.Length )
						m_Provisioner.EndChangeTitle( m_From, m_Entries[buttonID].m_Title, m_Entries[buttonID].m_Vendor );
				}
				else
				{
					m_Provisioner.CancelChangeTitle( m_From );
				}
			}
		}
	}

	public class ProvisionerGump : Gump
	{
		private Mobile m_From;
		private PlayerProvisioner m_Provisioner;
		...
		public void RenderDismissConfirmation()
		{
			...
			AddHtml( 170, 160, 380, 20, "Are you sure you want to dismiss your provisioner?", false, false );
		...
		private void RenderAppearanceCategories()
		{
			...
			if ( m_Provisioner.BodyValue != 0x340 && m_Provisioner.BodyValue != 0x402 ) {			
		...
		public ProvisionerGump( Mobile from, PlayerProvisioner provisioner ) : base( 0, 0 )
		{
			m_From = from;
			m_Provisioner = provisioner;
		...
		{
			if ( !m_Provisioner.IsOwner( m_From ) )
				return;
			...
			switch ( type )
			{
				...
				case 5: // Appearance category selection
				{
					switch ( index )
					{
						case 0: m_Provisioner.BeginChangeTitle( m_From ); break;
						case 1: m_Provisioner.BeginChangeAppearance( m_From ); break;
						case 2: m_Provisioner.ChangeGender( m_From ); break;
					}
