...
namespace Server.Mobiles
{
	...
	public class TinkerRumor
	{
		...
		public TinkerRumor( string message, string keyword )
		{
			m_Message = message;
			m_Keyword = keyword;
		}

		public static TinkerRumor Deserialize( GenericReader reader )
		{
			if ( !reader.ReadBool() )
				return null;

			return new TinkerRumor( reader.ReadString(), reader.ReadString() );
		}

		public static void Serialize( GenericWriter writer, TinkerRumor rumor )
		{
			if ( rumor == null )
			{
				writer.Write( false );
	...
	public class ManageTinkerEntry : ContextMenuEntry
	{
		private Mobile m_From;
		private PlayerTinker m_Tinker;

		public ManageTinkerEntry( Mobile from, PlayerTinker tinker ) : base( 6151, 12 )
		{
			m_From = from;
			m_Tinker = tinker;
		}
		...
		{
			m_Tinker.BeginManagement( m_From );
		}
	}

	public class PlayerTinker : Tinker
	{
		private Mobile m_Owner;
		private BaseHouse m_House;
		private string m_TipMessage;

		private TinkerRumor[] m_Rumors;
		...
		public TinkerRumor[] Rumors{ get{ return m_Rumors; } }
		...
		public PlayerTinker( Mobile owner, BaseHouse house ) : base()
		{
			m_Owner = owner;
			House = house;
			m_Rumors = new TinkerRumor[3];
		...
		public override void GetContextMenuEntries( Mobile from, List<ContextMenuEntry> list )
		{
			...
			if ( IsOwner( from ) && from.InLOS( this ) )
				list.Add( new ManageTinkerEntry( from, this ) );
		}
		...
		public void BeginManagement( Mobile from )
		{
			...
			from.SendGump( new TinkerGump( from, this ) );
		}
		...
		{
			from.SendGump( new TinkerTitleGump( from, this ) );
		}
		...
		{
			if ( Title == "the tinker" || Title == "the tinker" )
			{
		...
		public PlayerTinker( Serial serial ) : base( serial )
		{
		}
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			for ( int i = 0; i < m_Rumors.Length; ++i )
				TinkerRumor.Serialize( writer, m_Rumors[i] );
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				...
				case 0:
				{
					...
					m_Rumors = new TinkerRumor[reader.ReadEncodedInt()];

					for ( int i = 0; i < m_Rumors.Length; ++i )
						m_Rumors[i] = TinkerRumor.Deserialize( reader );
	...
	public class TinkerTitleGump : Gump
	{
		private Mobile m_From;
		private PlayerTinker m_Tinker;
		...
		public TinkerTitleGump( Mobile from, PlayerTinker tinker ) : base( 0, 0 )
		{
			m_From = from;
			m_Tinker = tinker;

			from.CloseGump( typeof( TinkerGump ) );
			from.CloseGump( typeof( TinkerTitleGump ) );
			...
			if ( buttonID > 0 )
			{
				...
				if ( buttonID > 0 )
				{
					...
					if ( buttonID >= 0 && buttonID < m_Entries.Length )
						m_Tinker.EndChangeTitle( m_From, m_Entries[buttonID].m_Title, m_Entries[buttonID].m_Vendor );
				}
				else
				{
					m_Tinker.CancelChangeTitle( m_From );
				}
			}
		}
	}

	public class TinkerGump : Gump
	{
		private Mobile m_From;
		private PlayerTinker m_Tinker;
		...
		public void RenderDismissConfirmation()
		{
			...
			AddHtml( 170, 160, 380, 20, "Are you sure you want to dismiss your tinker?", false, false );
		...
		private void RenderAppearanceCategories()
		{
			...
			if ( m_Tinker.BodyValue != 0x340 && m_Tinker.BodyValue != 0x402 ) {			
		...
		public TinkerGump( Mobile from, PlayerTinker tinker ) : base( 0, 0 )
		{
			m_From = from;
			m_Tinker = tinker;
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			switch ( type )
			{
				...
				case 5: // Appearance category selection
				{
					switch ( index )
					{
						case 0: m_Tinker.BeginChangeTitle( m_From ); break;
						case 1: m_Tinker.BeginChangeAppearance( m_From ); break;
						case 2: m_Tinker.ChangeGender( m_From ); break;
					}
