...
namespace Server.Mobiles
{
	...
	public class BankerRumor
	{
		...
		public BankerRumor( string message, string keyword )
		{
			m_Message = message;
			m_Keyword = keyword;
		}

		public static BankerRumor Deserialize( GenericReader reader )
		{
			if ( !reader.ReadBool() )
				return null;

			return new BankerRumor( reader.ReadString(), reader.ReadString() );
		}

		public static void Serialize( GenericWriter writer, BankerRumor rumor )
		{
			if ( rumor == null )
			{
				writer.Write( false );
	...
	public class ManageBankerEntry : ContextMenuEntry
	{
		private Mobile m_From;
		private PlayerBanker m_Banker;

		public ManageBankerEntry( Mobile from, PlayerBanker banker ) : base( 6151, 12 )
		{
			m_From = from;
			m_Banker = banker;
		}
		...
		{
			m_Banker.BeginManagement( m_From );
		}
	}

	public class PlayerBanker : Banker
	{
		private Mobile m_Owner;
		private BaseHouse m_House;
		private string m_TipMessage;

		private BankerRumor[] m_Rumors;
		...
		public BankerRumor[] Rumors{ get{ return m_Rumors; } }
		...
		public PlayerBanker( Mobile owner, BaseHouse house ) : base()
		{
			m_Owner = owner;
			House = house;
			m_Rumors = new BankerRumor[3];
		...
		public override void GetContextMenuEntries( Mobile from, List<ContextMenuEntry> list )
		{
			...
			if ( IsOwner( from ) && from.InLOS( this ) )
				list.Add( new ManageBankerEntry( from, this ) );
		}
		...
		public void BeginManagement( Mobile from )
		{
			...
			from.SendGump( new BankerGump( from, this ) );
		}
		...
		{
			from.SendGump( new BankerTitleGump( from, this ) );
		}
		...
		{
			if ( Title == "the banker" || Title == "the banker" )
			{
		...
		public PlayerBanker( Serial serial ) : base( serial )
		{
		}
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			for ( int i = 0; i < m_Rumors.Length; ++i )
				BankerRumor.Serialize( writer, m_Rumors[i] );
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				...
				case 0:
				{
					...
					m_Rumors = new BankerRumor[reader.ReadEncodedInt()];

					for ( int i = 0; i < m_Rumors.Length; ++i )
						m_Rumors[i] = BankerRumor.Deserialize( reader );
	...
	public class BankerTitleGump : Gump
	{
		private Mobile m_From;
		private PlayerBanker m_Banker;
		...
		public BankerTitleGump( Mobile from, PlayerBanker banker ) : base( 0, 0 )
		{
			m_From = from;
			m_Banker = banker;

			from.CloseGump( typeof( BankerGump ) );
			from.CloseGump( typeof( BankerTitleGump ) );
			...
			if ( buttonID > 0 )
			{
				...
				if ( buttonID > 0 )
				{
					...
					if ( buttonID >= 0 && buttonID < m_Entries.Length )
						m_Banker.EndChangeTitle( m_From, m_Entries[buttonID].m_Title, m_Entries[buttonID].m_Vendor );
				}
				else
				{
					m_Banker.CancelChangeTitle( m_From );
				}
			}
		}
	}
	...
	public class BankerGump : Gump
	{
		...
		public void RenderDismissConfirmation()
		{
			...
			AddHtml( 170, 160, 380, 20, "Are you sure you want to dismiss your banker?", false, false );
		...
		private void RenderAppearanceCategories()
		{
			...
			if ( m_Banker.BodyValue != 0x340 && m_Banker.BodyValue != 0x402 ) {			
		...
		public BankerGump( Mobile from, PlayerBanker banker ) : base( 0, 0 )
		{
			m_From = from;
			m_Banker = banker;
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			switch ( type )
			{
				...
				case 5: // Appearance category selection
				{
					switch ( index )
					{
						case 0: m_Banker.BeginChangeTitle( m_From ); break;
						case 1: m_Banker.BeginChangeAppearance( m_From ); break;
						case 2: m_Banker.ChangeGender( m_From ); break;
					}
