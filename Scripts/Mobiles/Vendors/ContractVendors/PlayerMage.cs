...
namespace Server.Mobiles
{
	...
	public class MageRumor
	{
		...
		public MageRumor( string message, string keyword )
		{
			m_Message = message;
			m_Keyword = keyword;
		}

		public static MageRumor Deserialize( GenericReader reader )
		{
			if ( !reader.ReadBool() )
				return null;

			return new MageRumor( reader.ReadString(), reader.ReadString() );
		}

		public static void Serialize( GenericWriter writer, MageRumor rumor )
		{
			if ( rumor == null )
			{
				writer.Write( false );
	...
	public class ManageMageEntry : ContextMenuEntry
	{
		private Mobile m_From;
		private PlayerMage m_Mage;

		public ManageMageEntry( Mobile from, PlayerMage mage ) : base( 6151, 12 )
		{
			m_From = from;
			m_Mage = mage;
		}
	...
	public class PlayerMage : Mage
	{
		...
		public MageRumor[] Rumors{ get{ return m_Rumors; } }
		...
		public PlayerMage( Mobile owner, BaseHouse house ) : base()
		{
			m_Owner = owner;
			House = house;
			m_Rumors = new MageRumor[3];
		...
		public override void GetContextMenuEntries( Mobile from, List<ContextMenuEntry> list )
		{
			...
			if ( IsOwner( from ) && from.InLOS( this ) )
				list.Add( new ManageMageEntry( from, this ) );
		}
		...
		public void BeginManagement( Mobile from )
		{
			...
			from.SendGump( new MageGump( from, this ) );
		}
		...
		{
			from.SendGump( new MageTitleGump( from, this ) );
		}
		...
		{
			if ( Title == "the mage" || Title == "the mage" )
			{
		...
		public PlayerMage( Serial serial ) : base( serial )
		{
		}
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			for ( int i = 0; i < m_Rumors.Length; ++i )
				MageRumor.Serialize( writer, m_Rumors[i] );
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				...
				case 0:
				{
					...
					m_Rumors = new MageRumor[reader.ReadEncodedInt()];

					for ( int i = 0; i < m_Rumors.Length; ++i )
						m_Rumors[i] = MageRumor.Deserialize( reader );
	...
	public class MageTitleGump : Gump
	{
		private Mobile m_From;
		private PlayerMage m_Mage;
		...
		public MageTitleGump( Mobile from, PlayerMage mage ) : base( 0, 0 )
		{
			m_From = from;
			m_Mage = mage;

			from.CloseGump( typeof( MageGump ) );
			from.CloseGump( typeof( MageTitleGump ) );
			...
			if ( buttonID > 0 )
			{
				...
				if ( buttonID > 0 )
				{
					...
					if ( buttonID >= 0 && buttonID < m_Entries.Length )
						m_Mage.EndChangeTitle( m_From, m_Entries[buttonID].m_Title, m_Entries[buttonID].m_Vendor );
				}
				else
				{
					m_Mage.CancelChangeTitle( m_From );
				}
			}
		}
	}
	...
	public class MageGump : Gump
	{
		...
		public void RenderDismissConfirmation()
		{
			...
			AddHtml( 170, 160, 380, 20, "Are you sure you want to dismiss your mage?", false, false );
		...
		private void RenderAppearanceCategories()
		{
			...
			if ( m_Mage.BodyValue != 0x340 && m_Mage.BodyValue != 0x402 ) {			
		...
		public MageGump( Mobile from, PlayerMage mage ) : base( 0, 0 )
		{
			m_From = from;
			m_Mage = mage;
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			switch ( type )
			{
				...
				case 5: // Appearance category selection
				{
					switch ( index )
					{
						case 0: m_Mage.BeginChangeTitle( m_From ); break;
						case 1: m_Mage.BeginChangeAppearance( m_From ); break;
						case 2: m_Mage.ChangeGender( m_From ); break;
					}
