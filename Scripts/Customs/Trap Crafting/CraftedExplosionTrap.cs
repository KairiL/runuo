...
{
	public class CraftedExplosionTrap : BaseTrap
	{
		...
		public override void OnTrigger( Mobile from )
		{
			...
			Effects.SendLocationParticles( EffectItem.Create( Location, Map, EffectItem.DefaultDuration ), 0x36B0, 10, 30, 5052 );
			Effects.PlaySound( Location, Map, 519 );

           		int MinDamage = 40 * (TrapPower / 100);
            		int MaxDamage = 70 * (TrapPower / 100);
			...
			if ( from.Alive )
			{
				...
				IPooledEnumerable eable = this.Map.GetMobilesInRange( new Point3D( Location ), 1 );
