		...
		internal class AccountGump : Gump
		{
			public AccountGump( Mobile from, NetState state, bool showOnlyCredits ) : base( 30, 20 )
			{
				...
				if ( !showOnlyCredits )
				{
					...
					AddLabel( 205, 180, NewsGump.kDataColor, (( (int)state.Flags & 0x10 ) != 0 ) ? "Samurai Empire" : (((int)state.Flags & 0x08 ) != 0) ? "Age of Shadows" : (((int)state.Flags & 0x04 ) != 0) ? "Blackthorn's Revenge" : (((int)state.Flags & 0x02 ) != 0 ) ? "Third Dawn" : (((int)state.Flags & 0x01 ) != 0 ) ? "Renaissance" : "The Second Age" );
